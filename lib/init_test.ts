import { addTemplate } from "./init.ts";
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      await addTemplate(tmpDir, "tasks.md");
      const originalContent = await Deno.readTextFile(
        join(import.meta.dirname!, "..", "templates", "tasks.md"),
      );
      const taskTemplateContent = await Deno.readTextFile(
        join(tmpDir, ".zk", "templates", "tasks.md"),
      );
...
Deno.test({
  ...
  async fn() {
    ...
    await assertRejects(() => addTemplate(tmpDir, "tasks.md"));
    await Deno.remove(tmpDir, { recursive: true });
  },
});
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const customizedTemplate = await Deno.readTextFile(
        join(tmpDir, ".zk", "templates", "tasks.md"),
      );
      await addTemplate(tmpDir, "tasks.md");
      const originalContent = await Deno.readTextFile(
